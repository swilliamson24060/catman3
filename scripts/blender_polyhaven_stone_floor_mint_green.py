...
def assign_material_to_selected_object(material_name: str, color_path: str, roughness_path: str = None, normal_path: str = None):
    ...
    if len(color_ramp.color_ramp.elements) >= 3:
        ...
        color_ramp.color_ramp.elements[0].color = (0.45, 0.62, 0.58, 1.0)
        color_ramp.color_ramp.elements[1].position = 0.6
        color_ramp.color_ramp.elements[1].color = (0.72, 0.88, 0.80, 1.0)
        color_ramp.color_ramp.elements[2].position = 1.0
        color_ramp.color_ramp.elements[2].color = (0.90, 0.98, 0.94, 1.0)
    links.new(color_tex.outputs["Color"], color_ramp.inputs["Fac"])

    pastel = nodes.new(type="ShaderNodeRGB")
    pastel.location = (-480, 40)
    if pastel.outputs:
        pastel.outputs[0].default_value = (0.76, 0.95, 0.88, 1.0)
    ...
    if roughness_path and Path(roughness_path).exists():
        ...
        if len(rough_ramp.color_ramp.elements) >= 3:
            ...
            rough_ramp.color_ramp.elements[0].color = (0.22, 0.22, 0.22, 1.0)
            rough_ramp.color_ramp.elements[1].position = 0.72
            rough_ramp.color_ramp.elements[1].color = (0.78, 0.80, 0.80, 1.0)
            rough_ramp.color_ramp.elements[2].position = 1.0
            rough_ramp.color_ramp.elements[2].color = (0.97, 0.99, 0.97, 1.0)
    ...
    if normal_path and Path(normal_path).exists():
        ...
        if strength_socket is not None:
            strength_socket.default_value = 0.12
...
def setup_three_point_lighting():
    ...
    add_light("KeyLight", key_loc, (1.1, 0.0, 0.7), 2200.0, (1.0, 0.97, 0.90))
    add_light("FillLight", fill_loc, (1.0, 0.0, 2.25), 1000.0, (0.78, 0.96, 0.92))
    add_light("RimLight", rim_loc, (0.95, 0.0, 3.14), 1400.0, (0.80, 0.98, 0.90))
    ...
    if bg:
        bg.inputs["Color"].default_value = (0.84, 0.97, 0.91, 1.0)
...
def main():
    ...
    base_dir = Path(bpy.path.abspath("//polyhaven_stone_floor_mint_green"))
    ...
    assign_material_to_selected_object(
        material_name=f"PH_{slug}_mint_green",
    ...
    print("Mint-green pastel cartoon material with 3-point lighting applied.")
...
if __name__ == "__main__":
    try:
        ...
        print("Poly Haven mint-green stone setup failed:", exc)
        raise
