...
def assign_material_to_selected_object(material_name: str, color_path: str, roughness_path: str = None, normal_path: str = None):
    ...
    if len(color_ramp.color_ramp.elements) >= 3:
        ...
        color_ramp.color_ramp.elements[0].color = (0.65, 0.72, 0.76, 1.0)
        color_ramp.color_ramp.elements[1].position = 0.58
        color_ramp.color_ramp.elements[1].color = (0.82, 0.88, 0.94, 1.0)
        color_ramp.color_ramp.elements[2].position = 1.0
        color_ramp.color_ramp.elements[2].color = (0.94, 0.98, 0.99, 1.0)
    links.new(color_tex.outputs["Color"], color_ramp.inputs["Fac"])

    pastel = nodes.new(type="ShaderNodeRGB")
    pastel.location = (-480, 40)
    if pastel.outputs:
        pastel.outputs[0].default_value = (0.78, 0.88, 0.97, 1.0)
    ...
    if roughness_path and Path(roughness_path).exists():
        ...
        if len(rough_ramp.color_ramp.elements) >= 3:
            ...
            rough_ramp.color_ramp.elements[1].color = (0.78, 0.78, 0.80, 1.0)
            rough_ramp.color_ramp.elements[2].position = 1.0
            rough_ramp.color_ramp.elements[2].color = (0.98, 0.98, 1.0, 1.0)
...
def setup_three_point_lighting():
    ...
    add_light("KeyLight", key_loc, (1.1, 0.0, 0.7), 2200.0, (1.0, 0.94, 0.88))
    add_light("FillLight", fill_loc, (1.0, 0.0, 2.25), 1100.0, (0.82, 0.90, 1.0))
    add_light("RimLight", rim_loc, (0.95, 0.0, 3.14), 1400.0, (0.85, 0.90, 1.0))
    ...
    if bg:
        bg.inputs["Color"].default_value = (0.86, 0.92, 0.97, 1.0)
...
def main():
    ...
    base_dir = Path(bpy.path.abspath("//polyhaven_stone_floor_baby_blue"))
    ...
    assign_material_to_selected_object(
        material_name=f"PH_{slug}_baby_blue",
    ...
    print("Baby-blue sun-washed cartoon material with 3-point lighting applied.")
...
if __name__ == "__main__":
    try:
        ...
        print("Poly Haven baby-blue stone setup failed:", exc)
        raise
