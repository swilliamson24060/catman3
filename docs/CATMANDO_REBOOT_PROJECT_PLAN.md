...
**Floor readability follow-up (2026-08-16):** The visible raised rectangular zone plates were replaced by one continuous world-space procedural ground material with broad grass variation, blended destination wear, and a softly irregular winding path. The deterministic MegaKit MultiMesh carpet is reduced to about 8k shorter tufts; a grass material normalizes the atlas's strongest yellow/orange variants into a cohesive green range. Weighted MegaKit rocks, ferns, flowers, mushrooms, and rare bushes now cluster along the path as well as the buffer ring. This also fixed the woodland-path exclusion checking ground south of the path instead of the path itself. Rendering remains batched and skipped in headless tests.
