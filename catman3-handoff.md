...
- **Elowen's cottage:** real GLB pieces (12-variant pastel granite stack set + foundation/roof), texture-corrected via image post-processing since the delivered assets shipped with a generic untinted material. Mara and Pip's homes are still flat colored placeholder boxes.
...
- **Ground:** flat per-zone color (`village_clearing.gd`'s `ZONE_COLORS`) — **not textured**. Two texture approaches were tried and both abandoned: (1) a painterly tileable grass texture (`environment/textures/clearing_grass/` etc.) read as visual noise once tiled across a 55m ground even with mipmaps; (2) repurposing MegaKit rock/pebble UV-atlas textures as a tiled ground fill also read as visibly tiled and bad, because the kit ships **no actual tileable ground texture** — its "diffuse" maps are all per-model UV atlases meant to wrap one prop, not repeat. The old texture files are still in the repo (unreferenced) in case a future attempt wants them.
- **Ground *coverage*** instead comes from real 3D vegetation: a dense `MultiMeshInstance3D`-based grass/clover "carpet" (~15,000 GPU-instanced tufts across the whole clearing, one draw call per species — individual scene-node instances at that density would be a real performance problem) plus clustered ferns/mushrooms/flowers/bushes and scattered boulders/pebbles placed in patches around trees (not uniform-random — patches read as natural undergrowth). All of this MegaKit-driven decoration is implemented in `scripts/world/nature_props.gd` (`NatureProps`, a shared static helper) and wired into `scenes/world/village_clearing.gd` / `scenes/world/woodland_route.gd`. It's skipped entirely when `DisplayServer.get_name() == "headless"` — pure visual dressing, zero gameplay effect, and it would otherwise slow down every test run.
