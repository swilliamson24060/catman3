...
func _build_clearing() -> void:
	...
	# stay flat placeholder blocks. Sized to roughly match the real Elowen
	# foundation pieces (~1.9m wide/deep, ~0.6m tall -- see
	# cottage/models/elowen/foundation_*.glb) rather than the old, much
	# larger guessed box, so the housing section doesn't look wildly
	# oversized next to the real cottage.
	_create_block("HomeMaraPlaceholder", Vector3(1.9, 0.6, 1.9), Vector3(-17.0, 0.3, -10.0), Color(0.78, 0.51, 0.45))
	_create_block("HomePipPlaceholder", Vector3(1.9, 0.6, 1.9), Vector3(-12.0, 0.3, -4.5), Color(0.45, 0.64, 0.78))
